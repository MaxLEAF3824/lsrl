...
export WANDB_MODE=disabled  # 设置为离线模式，避免自动上传到云端
cd /workspace/yiqiuguo/lsrl/
MODEL="Qwen/Qwen2.5-1.5B-Instruct"
DATA_FILE="/workspace/yiqiuguo/lsrl/gen_results/qwen2.5-1.5b-instruct_dapo_math_14k_en_openinstruct_rollout32_run20260512_a59b.jsonl"

echo "🚀 开始启动Latent Optimization & Distillation 训练..."
torchrun --nproc_per_node=4 /workspace/yiqiuguo/lsrl/latent_optimizer_v7.py \
    --model_name "$MODEL" \
    --file_path "$DATA_FILE" \
    --run_name hard_main_exp_qwen2.5-1.5b_$(basename "$0" .sh) \
    --vllm_gpus 0 1 2 3 \
    --big_batch_size 512 \
    --batch_size 1 \
    --chunk_size 512 \
