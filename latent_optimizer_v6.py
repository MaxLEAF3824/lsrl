...
def main():
    ...
    # [新增逻辑]：Rank 0 维护真正的全量 global_history，其它节点为空
    global_history = {}
    if rank == 0:
        ...
        for step in step_pbar:
            ...
            if len(active_uids) > 0:
                ...
                for batch_uids in inner_pbar:
                    ...
                    for i, uid in enumerate(batch_uids):
                        ...
                        for c_start in range(0, think_len, args.chunk_size):
                            ...
                            embeds_chunk = (
                                curr_think_list[i][:, 1 + c_start : 1 + c_end, :] if c_end < think_len else torch.cat([curr_think_list[i][:, 1 + c_start : think_len, :], embeds_end_think], dim=1)
                            )
                            target_ids_chunk = target_ids_think[:, c_start:c_end]
                            orig_embeds_chunk = model.get_input_embeddings()(target_ids_chunk).detach()
                            scaled_embeds_chunk = embeds_chunk * (torch.norm(orig_embeds_chunk, p=2, dim=-1, keepdim=True) / (torch.norm(embeds_chunk, p=2, dim=-1, keepdim=True) + 1e-8))

                            score_0_chunk = (h_chunk * scaled_embeds_chunk).sum(dim=-1)
                            mask_chunk = torch.zeros_like(logits_chunk, dtype=torch.bool).scatter_(2, target_ids_chunk.unsqueeze(-1), True)
                            ...
                            if step > 0:
                                s_reg_chunk = (kl_chunk.sum() / think_len) if args.reg_type == "kl" else (-logprobs_chunk_val / think_len)
                                ((args.kl_weight * s_reg_chunk) / batch_size_cur).backward()

                        s_gt, s_kl, s_lm = gt_loss.item(), kl_sum_val / think_len, -(logprobs_sum_val / think_len)
                        s_total = s_gt + args.kl_weight * (s_kl if args.reg_type == "kl" else s_lm)

                        step_metrics[uid] = {"total_loss": s_total, "gt_loss": s_gt, "kl_loss": s_kl, "lm_loss": s_lm}
                        epoch_gt_loss_sum += s_gt
                        epoch_kl_loss_sum += s_kl
                        epoch_lm_loss_sum += s_lm
                        epoch_total_loss_sum += s_total
                ...
                for uid in active_uids:
                    ...
                    if not (args.early_stop and step_metrics[uid]["gt_loss"] < args.early_stop_threshold):
